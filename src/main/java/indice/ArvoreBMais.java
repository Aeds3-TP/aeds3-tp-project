...
public class ArvoreBMais<T extends RegistroArvoreBMais<T>> {
    ...
    public ArvoreBMais(Constructor<T> c, int o, String na) throws Exception {
        try {
            ...
            // ---------------------------------------------------------------------
            // CORREÇÃO APLICADA AQUI: CRIAÇÃO AUTOMÁTICA DE DIRETÓRIOS
            // ---------------------------------------------------------------------
            File f = new File(nomeArquivo);
            File diretorioPai = f.getParentFile();
            if (diretorioPai != null && !diretorioPai.exists()) {
                diretorioPai.mkdirs(); // Cria as pastas dados/ingredientes/ se não existirem
            }
            // ---------------------------------------------------------------------
            ...
            throw new Exception("Erro ao criar ou abrir o arquivo da árvore B+ em: " + nomeArquivo);
        }
    }
    ...
    public ArrayList<T> readAll() throws Exception {
        ...
        // Agora percorre as folhas em sequência
        while (pagina != -1) {
            arquivo.seek(pagina);
            pa = new Pagina(construtor, ordem);
            buffer = new byte[pa.TAMANHO_PAGINA];
            ...
            for (T elem : pa.elementos) {
                lista.add(elem.clone());
            }
    ...
    // Atualiza um elemento na árvore.
    // O método de atualização assume que a CHAVE do objeto (usada no compareTo) 
    // não foi alterada, apenas os outros dados do registro (payload).
    public boolean update(T elem) throws Exception {
        ...
        // Se a árvore estiver vazia, não há o que atualizar
        if (raiz == -1)
            return false;
    ...
    // Método recursivo de atualização
    private boolean update1(T elem, long pagina) throws Exception {

        // Se chegou em uma página inexistente, o elemento não está na árvore
        if (pagina == -1) {
            return false;
        }
        ...
        // Encontra o ponto onde a chave deveria estar
        // Salta todos os elementos menores que a chave buscada
        int i = 0;
        while (i < pa.elementos.size() && elem.compareTo(pa.elementos.get(i)) > 0) {
            i++;
        }

        // Caso 1: Encontrou o elemento em uma página FOLHA
        // Verifica se é folha (filho -1) e se o elemento é igual (compareTo == 0)
        if (i < pa.elementos.size() && pa.filhos.get(0) == -1 && elem.compareTo(pa.elementos.get(i)) == 0) {
            
            // Substitui o elemento antigo pelo novo (atualizado) na lista da memória
            pa.elementos.set(i, elem);

            // Grava a página atualizada de volta no arquivo (disco)
            arquivo.seek(pagina);
            arquivo.write(pa.toByteArray());
        ...
        // Caso 2: Não encontrou na folha atual, ou não é uma folha.
        // Continua a busca recursiva.
        
        // Se já estamos numa folha e não encontramos (e não entramos no if acima),
        // então o registro não existe.
        if (pa.filhos.get(0) == -1) {
            return false;
        }

        // Se não é folha, decide para qual filho descer
        if (i == pa.elementos.size() || elem.compareTo(pa.elementos.get(i)) < 0)
